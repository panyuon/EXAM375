...
public class Ticket
{
	...
	private static final String [] SchengenAirportsCode = new String[] { "VIE","BRU","PRG","CPH","TLL","HEL","CDG","FRA",
	"MUC","ATH","BUD","KEF","CIA","RIX","VNO","LUX","MLA","AMS","OSL","WAW","LIS","LJU","KSC","MAD","ARN","BRN"};
	/**
	...
	*/
	public static boolean checkTicket( ArrayList<Flight> ticket, int maxFlightsCount,
	 int maxFlightTime, int maxLayoverTime, boolean hasSchengenVisa)
	{
		//Todo: add your code here
		for(int i = 0; i < ArrayList.length; i++){
			if(maximumFlightsCount <= ticket){
				return false;
			}			
			if(maxFlightTime/60 < 1){
				return false;
			}		
			if(maxlayoverTime/60 < 1){
				return false;
			}
			if(hasSchengenVisa == ticket && ticket >=1){
				return false;
			}	
	...
	*/
	public static boolean hasCyclicTrip(ArrayList<Flight> ticket)
	{
		//Todo : add your code here
		for(int i = 0; i <ArrayList.length; i++){
			if(ticket == 1 && ArrayList[i] == ticket)
			  return true
		}
